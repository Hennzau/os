...
/* RTL8852BD (8852B, rom_version 3) needs an eco 4 patch, which
 * rtl8852bu_fw.bin does not have (eco 1 and 2 only). Realtek's Windows driver
 * has it, as records each loaded at its own address. Adapted from
 * github.com/mihaits/rtl8852bd-bt-linux (reverse-engineered there).
 *
 * rtl_bt/rtl8852bd_eco4.bin: "E4RD", __le32 count, then per record __le32
 * load address, __le32 length, the bytes. Per record: 0xfc62 (write memory,
 * [0x21][addr][value]) aims the download buffer pointer at 0x801200cc to the
 * record's address, then 0xfc20 fragments with plain indices; one lone
...
static int rtl_eco4_cmd(struct hci_dev *hdev, u16 op, const u8 *buf, int len)
{
	...
	skb = __hci_cmd_sync(hdev, op, len, buf, HCI_INIT_TIMEOUT);
	if (IS_ERR(skb)) {
		rtl_dev_err(hdev, "eco4: cmd 0x%04x failed (%ld)", op,
			    PTR_ERR(skb));
		return PTR_ERR(skb);
	}
	status = skb->len ? skb->data[0] : -EIO;
	kfree_skb(skb);
	return status;
}

static int rtl_download_eco4(struct hci_dev *hdev)
{
	const struct firmware *fw;
	const u8 *p, *end;
	u8 *frag;
	...
	ret = request_firmware(&fw, "rtl_bt/rtl8852bd_eco4.bin", &hdev->dev);
	if (ret < 0) {
		rtl_dev_err(hdev, "eco4: rtl_bt/rtl8852bd_eco4.bin not found");
		return ret;
	}
	if (fw->size < 8 || memcmp(fw->data, "E4RD", 4)) {
		rtl_dev_err(hdev, "eco4: bad firmware container");
		ret = -EINVAL;
		goto out;
	}
	p = fw->data + 8;
	end = fw->data + fw->size;
	nrec = get_unaligned_le32(fw->data + 4);

	frag = kmalloc(RTL_ECO4_FRAG + 1, GFP_KERNEL);
	if (!frag) {
		ret = -ENOMEM;
		goto out;
	}
	...
	for (i = 0; i < nrec; i++) {
		...
		load = get_unaligned_le32(p);
		len = get_unaligned_le32(p + 4);
		p += 8;
		if (end - p < len) {
			ret = -EINVAL;
			goto out_frag;
		...
		setup[0] = 0x21;
		put_unaligned_le32(RTL_ECO4_BUFPTR, setup + 1);
		...
		for (off = 0; off < len; off += RTL_ECO4_FRAG) {
       ...
       */
			...
			idx = idx < 0x7f ? idx + 1 : 1;
		}
	}

	frag[0] = 0x80;
	rtl_eco4_cmd(hdev, 0xfc20, frag, 1);
	rtl_dev_info(hdev, "RTL8852BD eco4 firmware loaded (%u records)", nrec);
	ret = 0;
				...
				struct btrtl_device_info *btrtl_dev)
{
	...
	/* No eco 4 in rtl8852bu_fw.bin: the v2 parser finds nothing (-ENODATA). */
	if (ret == -ENODATA && btrtl_dev->ic_info &&
	    btrtl_dev->ic_info->lmp_subver == RTL_ROM_LMP_8852A &&
	    btrtl_dev->ic_info->hci_rev == 0xb &&
	    btrtl_dev->ic_info->hci_bus == HCI_USB &&
	    btrtl_dev->rom_version == 3) {
		rtl_dev_info(hdev, "RTL8852BD: eco4 download");
		ret = rtl_download_eco4(hdev);
		goto out;
	}
	if (ret < 0)
		goto out;
...
MODULE_FIRMWARE("rtl_bt/rtl8852bd_eco4.bin");
